...
public abstract class EntityLivingBase extends Entity
{
    ...
    /** Entity head pitch yaw */
    public float rotationPitchHead;

    /** Entity head rotation yaw at previous tick */
    public float prevRotationYawHead;

    /** Entity head pitch yaw at previous tick */
        ...
        if (this.isServerWorld() || this.canPassengerSteer())
        {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    {
                        boolean ncpff = false;//Xero.moduleManager.getModuleByClass(Speed.class).isToggled();
                        float f6 = (!ncpff ? 0.91F : 0f);
                        BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain(this.posX, this.getEntityBoundingBox().minY - 1.0D, this.posZ);

                        if (this.onGround)
                        {
                            f6 = this.world.getBlockState(blockpos$pooledmutableblockpos).getBlock().slipperiness * (!ncpff ? 0.91F : 0f);
                        }
                        ...
                        if (this.onGround)
                        {
                            f6 = this.world.getBlockState(blockpos$pooledmutableblockpos.setPos(this.posX, this.getEntityBoundingBox().minY - 1.0D, this.posZ)).getBlock().slipperiness * (!ncpff ? 0.91F : 0.913f);
                        }
