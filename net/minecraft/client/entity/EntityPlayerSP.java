...
import fun.archware.impl.events.*;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        EventPreUpdate eventPreUpdate = new EventPreUpdate(posX, posY, posZ, rotationYaw, rotationPitch, onGround, getEntityBoundingBox().minY);
        eventPreUpdate.call();
        for(final String[] key : ScriptManager.REGISTERED_HANDLERS.keySet()){
            if(ArchWare.moduleManager.getModuleByName(key[0]).isToggled()
                    && key[1].equals("preupdate")){
                ScriptManager.REGISTERED_HANDLERS.get(key).call();
            }
        }
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.isRiding())
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0D, this.motionZ, eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
                flag2 = false;
            }
            else if (flag2 && flag3)
            {
    ...
    {
        if(ArchWare.moduleManager.getModuleByClass(NoPush.class).isToggled()) return false;
        if (this.noClip)
        {
            return false;
        }
    ...
    public void setSneak(boolean state){
        if(movementInput != null && !this.sleeping){
    ...
    public void onLivingUpdate()
    {
        ...
        {
            EventSlowdown eventSlowdown = new EventSlowdown(movementInput.moveStrafe, movementInput.field_192832_b);
            eventSlowdown.call();
