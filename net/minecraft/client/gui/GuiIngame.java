...
import fun.archware.impl.modules.render.CoolFeatureBtw;
import fun.archware.impl.modules.render.CrossHair;
import fun.archware.impl.modules.render.NoScoreboard;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        eventRender2D.call();
        for(final String[] key : ScriptManager.REGISTERED_HANDLERS.keySet()){
            if(ArchWare.moduleManager.getModuleByName(key[0]).isToggled()
            && key[1].equals("render2d")){
                try{
                    ScriptManager.REGISTERED_HANDLERS.get(key).call();
                }catch(final LuaError e){
                    ArchWare.moduleManager.getModuleByName(key[0]).toggle();
                }
            }
        }
    ...
    {
        if(ArchWare.moduleManager.getModuleByClass(CrossHair.class).isToggled() || ArchWare.moduleManager.getModuleByClass(CoolFeatureBtw.class).isToggled()) return;
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks)
    {
        ...
        {
            if(ArchWare.moduleManager.getModuleByName("CustomHotbar").isToggled()){
                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                ...
                if(String.valueOf(hotBar).equals("NaN")) hotBar = i - 90 + entityplayer.inventory.currentItem * 20;
                float f = this.zLevel;
                this.zLevel = -90.0F;
                RenderUtil.drawRect(0, sr.getScaledHeight() - 22, sr.getScaledWidth(), 22, Integer.MIN_VALUE);
                hotBar = (float) Interpolator.EASE_BOTH.interpolate(hotBar, i - 90 + entityplayer.inventory.currentItem * 20, (0.15*100)/ Minecraft.getDebugFPS());

                RenderUtil.drawRect(hotBar, sr.getScaledHeight() - 22, 21, 22, new Color(0, 0, 0, 155).hashCode());
    ...
    {
        if(ArchWare.moduleManager.getModuleByClass(NoScoreboard.class).isToggled()) return;
        ...
        for (Score score1 : collection)
        {
            ...
            if (j == collection.size())
            {
                ...
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - this.getFontRenderer().FONT_HEIGHT - 2, RenderUtil.getRainbow(500, 5000).hashCode());
                FontUtil.drawString(s3, l1 + i / 2 - (int)FontUtil.getStringWidth(s3) / 2, k - FontUtil.getFontHeight() - 1.5, 553648127);
            }
        }
    }
    ...
    {
        if(ArchWare.settingManager.getSettingById("NoRenderPumpkin").getValueBoolean() && ArchWare.moduleManager.getModuleByName("NoRender").isToggled()) return;
        GlStateManager.disableDepth();
    ...
    private void renderPortal(float timeInPortal, ScaledResolution scaledRes)
    {
        ...
        if(ArchWare.settingManager.getSettingById("NoRenderPortal").getValueBoolean() && ArchWare.moduleManager.getModuleByName("NoRender").isToggled()) return;
        GlStateManager.disableAlpha();
        GlStateManager.disableDepth();
        GlStateManager.depthMask(false);
        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
