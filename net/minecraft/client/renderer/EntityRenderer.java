...
import fun.archware.impl.modules.render.NameTags;
import fun.archware.impl.modules.render.NoHurt;
import fun.archware.impl.modules.render.SmoothCamera;
import fun.archware.impl.modules.render.ViewClip;
import javafx.animation.Interpolator;
import fun.archware.ArchWare;
import fun.archware.impl.events.EventRender3D;
import fun.archware.notifications.NotificationRenderer;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public static double cameraZoom = 1.0D;
    ...
    public void updateRenderer()
    {
        ...
        //this.thirdPersonDistance = ArchWare.moduleManager.getModuleByClass(CameraDistance.class).isToggled() ? 13 : 0;
        //this.thirdPersonDistancePrev = ArchWare.moduleManager.getModuleByClass(CameraDistance.class).isToggled() ? 13 : 4;
        //this.clipDistance = 20;
    ...
    {
        if(ArchWare.moduleManager.getModuleByClass(NoHurt.class).isToggled()) return;
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        {
            if(ArchWare.moduleManager.getModuleByClass(SmoothCamera.class).isToggled()){
                d3 = (float)Interpolator.EASE_IN.interpolate(d3, this.thirdPersonDistancePrev + (4.0F - this.thirdPersonDistancePrev) * partialTicks, (0.12 * 100) / Minecraft.getDebugFPS());

            }else{
                d3 = this.thirdPersonDistancePrev + (4.0F - this.thirdPersonDistancePrev) * partialTicks;
            }
            if (this.mc.gameSettings.debugCamEnable)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if(!ArchWare.moduleManager.getModuleByClass(ViewClip.class).isToggled()){
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        NotificationRenderer.render(partialTicks, new ScaledResolution(mc));
        this.frameFinish();
    ...
    public static void drawNameplate(FontRenderer fontRendererIn, String str, float x, float y, float z, int verticalShift, float viewerYaw, float viewerPitch, boolean isThirdPersonFrontal, boolean isSneaking, Entity entity)
    {
        ...
        if(!ArchWare.moduleManager.getModuleByClass(NameTags.class).isToggled()){
            ...
            int health = (int)((((EntityPlayer)entity).getHealth() / ((EntityPlayer)entity).getMaxHealth()) * 100);
            ...
            BufferBuilder bufferbuilder = tessellator.getBuffer();
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR);
            bufferbuilder.pos(-i - 1, -1 + verticalShift, 0.0D).color(0.0F, 0.0F, 0.0F, 100).endVertex();
            bufferbuilder.pos(-i - 1, 8 + verticalShift, 0.0D).color(0.0F, 0.0F, 0.0F, 100).endVertex();
            bufferbuilder.pos(i + 1, 8 + verticalShift, 0.0D).color(0.0F, 0.0F, 0.0F, 100).endVertex();
            bufferbuilder.pos(i + 1, -1 + verticalShift, 0.0D).color(0.0F, 0.0F, 0.0F, 100).endVertex();
            tessellator.draw();
            GlStateManager.enableTexture2D();
            ...
            GlStateManager.depthMask(true);
            fontRendererIn.drawString(str + TextFormatting.GREEN + " " + health + "%", -fontRendererIn.getStringWidth(str + " " + health + "%") / 2, verticalShift, -1);
            GlStateManager.enableDepth();
            GlStateManager.enableLighting();
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            GlStateManager.popMatrix();
